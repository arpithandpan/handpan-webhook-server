...
async function invoiceFeePayment(supabase, { feeId, studentName, student, classes, amountMajor, currency, payment, today, feeMonth, payerEmail, payerPhone, listAmount, discountPct }) {
  ...
  // With a package discount, the classes line carries the full price and a
  // second negative line carries the discount, so the total is still exactly
  // what was paid.
  const gross = Number(listAmount) > amountMajor ? Number(listAmount) : amountMajor;
  const discount = Math.round((gross - amountMajor) * 100) / 100;
  const qty = classes > 0 ? classes : 1;
  const rate = Math.round((gross / qty) * 100) / 100;
  const clean = Math.round(rate * qty * 100) === Math.round(gross * 100);
  const lines = clean
